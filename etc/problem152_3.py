...
# For every prime p >= 5, we consider the multiples of p
# independently. For the reasons stated in problem152_1.raku, the
# multiples of p that we sum must result in a fraction that does not
# contain a p in its denominator.
#
# (In principle, we could do this for p=3, but brute forcing that is
# slower than just brute forcing the remaining values at the end)
#
# We build up sets of possible "sums of multiples of p", then we
# combine those sets using a special operation. A set of multiples of
# p and a set of multiples of q are compatible if they agree on all
# multiples of pq (i.e. anytime the two sets *could* intersect, they
# agree). We use this structural approach to build up candidate
# solutions. Then, once we have those, we brute force all possible
# sums of numbers whose prime factors are only 2 and 3.
...
def analyze_prime(p: int) -> Iterable[SetInUniverse]:
    ...
    for candidate in powerset(multiples):
        ...
        if current_sum.denominator % p != 0:
